...
- A persistent spaced-retry queue automatically adds every wrong category Quiz MCQ for five successful recall reviews. The category screen's **Review Queue** card shows both the queued-question count and the total reviews remaining, and opens dedicated all-subject practice whenever desired. Dedicated practice uses saved shuffled rounds: each waiting MCQ appears once before another round begins, the previous MCQ is moved to the end of the next round for the longest available gap, and every saved remaining count produces exactly that many required correct appearances. The unfinished round survives refreshes and device changes.
- Every category Quiz (including Difficult Quiz) can also present due reviews from the full queue, regardless of which category or Basic Computer source created them. A global randomized cadence leaves five or six newly answered main Quiz questions between review prompts, so multiple overdue items cannot interrupt after every question. Learn and Custom Paper never show or create reviews. A wrong review adds exactly five more required correct appearances, while existing saved counts continue from their exact value.
- Finishing a category Quiz opens Results without forcing the retry queue to finish. Any remaining reviews stay saved for the dedicated **Review Queue** flow or a later Quiz in any category; **Later** can postpone an embedded review without changing the main Quiz score.
