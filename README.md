...
Existing saved retry counts are never reset during the first cloud migration. The correct-answer confirmation rule applies only to Quiz answers submitted after this release; old correct-answer history is not scanned or retroactively queued. The `+2` rule applies only when an MCQ is answered incorrectly in a new Quiz or finite queue review; a previously saved item continues from its exact stored remaining count. A checked review saved under the old `+5` rule keeps that pending outcome after refresh.
