...
- A persistent spaced-retry queue automatically adds every wrong category Quiz MCQ for five successful recall reviews. Every category Quiz (including Difficult Quiz) can present due reviews from the full queue, regardless of which category or Basic Computer source created them. Learn and Custom Paper never show or create reviews. One due review appears after every three newly answered Quiz questions, and a wrong retry adds five more reviews. Existing saved queue counts are preserved and continue counting down normally.
- Finishing a category Quiz opens Results without forcing the retry queue to finish. Any remaining reviews stay saved and continue during a later Quiz in any category; **Later** can postpone a review without changing the main Quiz score.
