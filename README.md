...
The finite Queue count baseline is a one-time migration: every item already present is set to exactly 5 whether its old count was lower or higher. A saved version marker prevents refresh, browser restart or another synced device from resetting those counts to 5 again. Question IDs, due timing, active attempts and highest-count-first selection remain intact. After that migration, every new wrong Quiz or finite Queue answer adds 2 and every correct Queue review removes 1. A checked review saved under the old `+5` rule keeps that pending atomic outcome after refresh.
