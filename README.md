...
- A persistent spaced-retry queue automatically adds every wrong category Quiz MCQ for five successful recall reviews. An always-visible header **Queue** button shows the total reviews remaining and opens dedicated all-subject practice from every protected screen; the category screen's larger **Review Queue** card also shows the queued-question and remaining-review totals. Dedicated practice uses saved shuffled rounds: each waiting MCQ appears once before another round begins, the previous MCQ is moved to the end of the next round for the longest available gap, and every saved remaining count produces exactly that many required correct appearances. The unfinished round survives refreshes and device changes.
- Every category Learn or Quiz session, including Difficult Learn/Quiz, can present due reviews from the full queue regardless of which category or Basic Computer source created them. A global randomized cadence leaves five or six newly completed main Learn/Quiz questions between review prompts; when several MCQs are due, the next one is selected from a shuffled set and the previous review is avoided while another due question is available. Learn consumes pending reviews but never creates new ones; Custom Paper never inserts or automatically shows reviews, although its header Queue button remains available. A wrong review adds exactly five more required correct appearances, while existing saved counts continue from their exact value.
...
The browser smoke test requires local Chrome or Edge. It exercises all five category modes (Learn, Quiz, Difficult, Study Notes and Detailed Learning) plus Custom Paper at mobile width. Coverage includes exact inclusive range selection, invalid-range protection, Important/Difficult filtered ranges, Previous/Next answer restoration, stable Learn/category-Quiz question order, shuffled ordinary answer choices, answer remapping, random 100-question paper construction, negative marking, Correct/Wrong review lists, range-preserving Restart and **Start Quiz**, changed-Difficult-list protection, the persistent mark/unmark control, v8 Continue restoration, v7/v6 migration to the all-source scope, invalid/tampered-source recovery, corrupt/stale or tampered-pool storage recovery, and completed-session clearing. It pins all four Basic Computer source counts and labels, filtered Quick Notes search/order, Learn/Quiz/Important/Difficult range pools, unchanged answer scoring, persisted source selection, and cross-category/source retry continuity. It verifies the always-visible header Queue control, the category card's unique-question and remaining-review totals, dedicated randomized round practice, exact saved repetition counts, shuffled selection among multiple overdue embedded reviews, Learn/Quiz five-or-six-question embedded cadence, global throttling, correct decrement, wrong-answer +5 rule, preservation of legacy saved remaining counts, Later postponement, reload/Continue idempotency, Urdu and position-dependent option rendering, mobile dialog focus, and complete separation from main-session progress. It also verifies cross-category/source review rendering, Difficult Learn/Quiz eligibility, Custom Paper automatic-review suppression with manual Queue access, saved active-attempt recovery, immediate Results with pending reviews, and continuation of unfinished work in a later Learn/Quiz session. It also covers all-category Quick Notes order and answers, visible stable question numbers, 100-row progressive loading, search, Clear, Important filtering, Urdu RTL rendering, mobile overflow, focus return, the nested Detailed GK Stories flow, Detailed Learning category isolation/topic filters/search/Clear/mobile layout/Urdu direction, and the guarantee that either reading mode leaves local storage unchanged.
