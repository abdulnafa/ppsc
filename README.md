...
- An accessible Light/Dark appearance control is available on both the Google sign-in screen and the protected app header. The selected theme is applied before the page paints, stays saved in that browser, follows the device colour preference until a choice is made, and keeps all notes, forms, quiz states, dialogs and result panels readable.
...
- A persistent spaced-retry queue now gives each newly answered category Quiz MCQ a confirmation cycle. On this release, every already-saved finite Queue MCQ is moved to an exact five-review baseline once; afterward a correct review removes one and a wrong answer creates two reviews or adds exactly two to its existing count. A correct main Quiz answer never decrements, resets or duplicates an already queued item—the count changes only inside Review Queue practice. An always-visible header **Queue** button shows the total finite reviews remaining and opens dedicated all-subject practice from every protected screen; the category screen's larger **Review Queue** panel shows the same totals plus a saved **3 / 5 / 10 other MCQs** repeat-gap selector (default: 5). Dedicated practice selects the highest-count MCQ that is outside the selected recent-question window, so a newly wrong high-count MCQ does not immediately repeat. Small queues cycle every available alternative and then use the widest possible gap without deadlocking. Every post-baseline count, selected gap, recent-question history and active attempt survives refreshes and device changes.
...
The finite Queue count baseline is a one-time migration: every item already present is set to exactly 5 whether its old count was lower or higher. A saved version marker prevents refresh, browser restart or another synced device from resetting those counts to 5 again. Question IDs, due timing and active attempts remain intact. The dedicated Queue's selected repeat gap and last ten completed Queue-question IDs are stored in the same offline-first record; older records safely receive the default five-question gap without losing any counts. After the count migration, every new wrong Quiz or finite Queue answer adds 2 and every correct Queue review removes 1. A checked review saved under the old `+5` rule keeps that pending atomic outcome after refresh.
