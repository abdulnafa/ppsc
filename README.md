...
- A persistent spaced-retry queue automatically adds every wrong Quiz or Custom Paper MCQ for five successful recall reviews. One due review appears after every three newly processed main questions in Learn, Quiz or Custom Paper; a wrong retry adds five more reviews, while **Later** postpones it without changing its remaining count.
...
Difficult marks, the compact active-session checkpoint, and the spaced-retry queue are stored in that browser and device using local storage. They survive normal reloads and visits; clearing the site's browser data removes them. Completed sessions are removed from Continue automatically, while unfinished retry reviews remain available across results, categories, reloads and later Learn or Quiz sessions. Retry answers never alter the current session's question number, answer history, Quiz score or Custom Paper marks.
